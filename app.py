...
# 處理查詢任務狀態指令
def handle_status_query(user_id, trello_id):
    if "狀態" in text or "進度" in text:  # 判斷是否為查詢任務狀態或進度
        try:
            ...
            logger.info(f"用戶 {user_id} 查詢任務狀態，Trello ID: {trello_id}") # 記錄查詢任務狀態訊息
...
def callback():
    ...
    for event in events:  # 迭代處理每一個事件
        if event['type'] == 'message' and event['message']['type'] == 'text':  # 判斷事件類型是否為文字訊息
            ...
            if handle_status_query(user_id, bindings.get(user_id)): # 處理狀態查詢指令
                continue # 指令已處理，繼續下一個事件
