            ...
            if "狀態" in text or "進度" in text: # 判斷是否為查詢任務狀態或進度
                ...
                try:
                    if start_date_str: # 如果有開始日期字串
                        ...
                        start_date_str = start_datetime.strftime('%Y-%m-%d') # 將解析後的 datetime 物件格式化為 YYYY-MM-DD 字串，方便記錄和使用
                        logger.info(f"NLP 解析開始日期成功: {start_date_str}") # 記錄 NLP 解析成功訊息
                    if due_date_str: # 如果有截止日期字串
                        due_datetime = parser.parse(due_date_str, fuzzy=True) # 使用 fuzzy=True 允許模糊日期解析
                        due_date_str = due_datetime.strftime('%Y-%m-%d') # 將解析後的 datetime 物件格式化為 YYYY-MM-DD 字串，方便記錄和使用
...
def get_user_trello_tasks(trello_member_id):
    try:
        ...
        for card in user_cards: # 迭代處理每一張卡片
            ...
            try:
                if due: # 如果有截止日期
                    due = datetime.datetime.fromisoformat(due).strftime('%Y-%m-%d %H:%M') # 將 ISO 8601 格式的日期字串轉換為 YYYY-MM-DD HH:MM 格式
